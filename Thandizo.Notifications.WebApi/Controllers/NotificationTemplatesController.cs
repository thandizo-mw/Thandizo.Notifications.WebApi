...
namespace Thandizo.Notifications.WebApi.Controllers
{
    ...
    {
        INotificationTemplateService _service;
        ...
        public async Task<IActionResult> GetById([FromQuery] int templateId)
        {
            var response = await _service.Get(templateId);
        ...
        [ValidateModelState]
        public async Task<IActionResult> Add([FromBody]NotificationTemplateDTO notificationTemplate)
        {
            var outputHandler = await _service.Add(notificationTemplate);
        ...
        {
            var outputHandler = await _service.Delete(templateId);
            if (outputHandler.IsErrorOccured)
            {
                return BadRequest(outputHandler.Message);
            }
